...
begin
  ...
  if _received then
    ...
    limit 1;
    if method_id is null then raise exception 'Forma de pagamento não encontrada: %', coalesce(_payment_method_code,'(não informada)'); end if;
  else
    method_id := null;
  end if;
  ...
  if center_id is null and _service_id is not null then
    ...
    where scc.service_id = _service_id
