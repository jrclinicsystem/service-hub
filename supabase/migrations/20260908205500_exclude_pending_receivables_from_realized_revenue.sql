-- Valores pendentes são contas a receber e não receita realizada.
-- Receita, líquido, comissões e resultado passam a considerar apenas entradas recebidas.
-- Relatórios sem filtro de status consideram entradas recebidas e despesas pagas.
...
begin
  ...
  range_commissions as (
    select pc.* from public.professional_commissions pc
  ...
  union all select 'clinic_result', (coalesce((select sum(net_amount) from range_entries),0) - coalesce((select sum(amount) from range_expenses),0))::numeric
  union all select 'available_balance', (coalesce((select sum(net_amount) from range_entries),0) - coalesce((select sum(amount) from range_expenses),0))::numeric
...
begin
  ...
  with filtered as (
    ...
    where l.business_date between _from and _to
      ...
      and ((_status is not null and l.status=_status) or (_status is null and ((l.record_type='entry' and l.status='received') or (l.record_type='expense' and l.status='paid'))))
  ),
  grouped as (
    ...
    select 'fortnight', to_char(date_trunc('month',f.business_date)::date,'YYYY-MM')||case when extract(day from f.business_date)<=15 then '-1' else '-2' end,
      ...
      coalesce(sum(f.result_amount),0)::numeric
    from filtered f
    group by date_trunc('month',f.business_date)::date, case when extract(day from f.business_date)<=15 then 1 else 2 end, to_char(f.business_date,'MM/YYYY'), extract(day from f.business_date)<=15
    union all
    ...
    select 'professional', coalesce(f.professional_id::text,'sem-profissional'), coalesce(f.professional_name_snapshot,'Sem profissional'),
      count(*)::bigint, coalesce(sum(f.gross_amount),0)::numeric, coalesce(sum(f.fee_amount),0)::numeric, coalesce(sum(f.net_amount),0)::numeric,
      coalesce(sum(f.commission_amount),0)::numeric, coalesce(sum(f.clinic_amount),0)::numeric, 0::numeric, coalesce(sum(f.net_amount),0)::numeric
    ...
    select 'expense_category', coalesce(f.category_id::text,'sem-categoria'), coalesce(f.category_name,'Sem categoria'),
      count(*)::bigint, 0::numeric, 0::numeric, 0::numeric, 0::numeric, 0::numeric, coalesce(sum(f.expense_amount),0)::numeric, coalesce(sum(f.result_amount),0)::numeric
    ...
    select 'cost_center', coalesce(f.cost_center_id::text,'sem-centro'), coalesce(f.cost_center_name,'Sem centro de custo'),
      count(*)::bigint, coalesce(sum(f.gross_amount),0)::numeric, coalesce(sum(f.fee_amount),0)::numeric, coalesce(sum(f.net_amount),0)::numeric,
      coalesce(sum(f.commission_amount),0)::numeric, coalesce(sum(f.clinic_amount),0)::numeric, coalesce(sum(f.expense_amount),0)::numeric, coalesce(sum(f.result_amount),0)::numeric
