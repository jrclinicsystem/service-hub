...
const description =
  "Escolha o serviço, profissional, horário e a forma de pagamento do agendamento.";
...
function Agendar() {
  ...
  const selectedProfessional =
    professionals.find((item: any) => item.id === professionalId) ?? professionals[0];
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`jrclinic-public-booking-slots-${professionalId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  const canConfirm = Boolean(
    service && professionalId && day && time && name.trim() && email.trim() && validWhatsApp,
  );
  ...
  const confirm = async () => {
    ...
    try {
      ...
      toast.info(
        created.paymentKind === "full"
          ? "Última etapa: pagamento integral"
          : "Última etapa: pagar o sinal",
        {
          description:
            created.paymentKind === "full"
      ...
      toast.error(
        error instanceof Error ? error.message : "Não foi possível preparar o agendamento.",
      );
    } finally {
      setBusy(false);
    }
      ...
      <div className="min-h-screen">
        ...
        <main className="mx-auto max-w-3xl px-4 py-16 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            A clínica ainda não publicou serviços para agendamento.
          </p>
        </main>
        <SiteFooter />
      </div>
  ...
  return (
    <div className="min-h-screen w-full max-w-[100dvw] overflow-x-clip">
      ...
      <main className="mx-auto box-border w-full max-w-[1440px] px-4 pb-44 pt-6 sm:px-8 sm:pb-32 sm:pt-10 lg:pb-12">
        ...
        <h1 className="mt-1 text-[28px] font-semibold leading-tight sm:mt-2 sm:text-4xl">
          Reserve seu horário
        </h1>
        <p className="mt-2 max-w-[65ch] text-sm leading-relaxed text-muted-foreground sm:mt-3 sm:text-base">
          Escolha o atendimento e depois decida entre pagar {depositLabel} online, quitar tudo
          online ou pagar presencialmente.
        </p>
          ...
          <div className="space-y-4 sm:space-y-8">
              ...
              <div className="mt-4">
                ...
                >
                  <SelectTrigger id="profissional" className="mt-2 h-11 rounded-xl">
                    <SelectValue
                      placeholder={
                        service ? "Escolha o profissional" : "Selecione um serviço primeiro"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="mt-3 flex items-center justify-between rounded-xl bg-secondary/60 px-3 py-2.5 sm:hidden">
                <div>
                  <p className="text-xs font-medium">
                    {service
                      ? (selectedProfessional?.name ?? "Escolha o profissional")
            ...
            <section className="rounded-2xl border border-border bg-card p-4 shadow-soft sm:p-6">
                ...
                {days.map((item) => {
                  ...
                  return (
                    <button
                      key={item.key}
                      type="button"
                      disabled={unavailable || !service || !professionalId}
                      onClick={() => {
                        setDay(item.key);
                        setTime(null);
                        resetPreparedPayment();
                      }}
                      className={`w-[54px] shrink-0 rounded-xl border px-2 py-2.5 text-center transition disabled:opacity-35 sm:w-auto ${day === item.key && service && !unavailable ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background hover:bg-secondary"}`}
                    >
              ...
              <p className="mt-5 text-xs font-medium text-muted-foreground">
                Horários disponíveis para {selectedProfessional?.name ?? "o profissional"}
              </p>
              {slotsLoading ? (
                <div className="mt-3 rounded-xl bg-secondary/60 px-3 py-3 text-xs text-muted-foreground">
                  Carregando horários desta data...
                </div>
              ) : (
                <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {displayedTimeSlots.map((slot: any) => (
                    <button
                      key={slot.slot}
                      type="button"
                      disabled={!professionalId || !service}
                      onClick={() => {
                        setTime(slot.slot);
                        resetPreparedPayment();
                      }}
              ...
              {professionalId && !slotsLoading && displayedTimeSlots.length === 0 ? (
                <p className="mt-3 text-xs text-destructive">
                  Não há horários liberados para esta data.
                </p>
              ) : null}
            </section>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 sm:gap-4">
                ...
                <div className="sm:col-span-2">
                  <Label htmlFor="telefone">
                    WhatsApp <span className="font-semibold text-destructive">*</span>
                  </Label>
                  <Input
                    id="telefone"
                    inputMode="tel"
                    ...
                    className={`mt-2 h-11 rounded-xl ${phone && !validWhatsApp ? "border-destructive" : ""}`}
                  />
                  {phone && !validWhatsApp ? (
                    ...
                    <p className="mt-1.5 text-[11px] text-muted-foreground">
                      Obrigatório para confirmações e lembretes do seu agendamento.
                    </p>
                  )}
                </div>
                <div className="sm:col-span-2">
                  ...
                  <Textarea
                    ...
                    className="mt-2 min-h-[82px] resize-none rounded-xl"
                  />
                </div>
              </div>
            </section>
            ...
            <div className="sticky top-[120px] rounded-2xl border border-border bg-card p-6 shadow-soft">
              ...
              <dl className="mt-4 space-y-3 text-sm">
                <div className="flex justify-between gap-4">
                  ...
                  <dd className="text-right font-medium">{service?.name ?? "Não selecionado"}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-muted-foreground">Profissional</dt>
                  <dd className="text-right font-medium">{selectedProfessional?.name ?? "—"}</dd>
                </div>
                <div className="flex justify-between gap-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xl font-semibold">{service ? formatPrice(total) : "—"}</span>
              </div>
              <div className="mt-3 rounded-xl bg-primary-soft/60 px-3 py-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-semibold text-primary">
                    {service ? formatPrice(paymentNow) : "—"}
                  </span>
                </div>
                <div className="mt-1 flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">Restante</span>
                  <span className="font-medium">{service ? formatPrice(remaining) : "—"}</span>
                </div>
              </div>
              <p className="mt-3 text-xs text-muted-foreground">
                {!service
                  ? "Selecione o serviço para calcular os valores."
                  : paymentChoice === "deposit"
                    ? `${depositLabel} online pela InfinitePay.`
                    : paymentChoice === "full"
                      ? "Pagamento integral online pela InfinitePay."
                      : "Pagamento integral presencial na clínica."}
              </p>
              <Button
                size="lg"
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-base font-semibold leading-tight text-primary">
              {service
                ? paymentChoice === "onsite"
                  ? "Na clínica"
                  : formatPrice(paymentNow)
                : "—"}
            </p>
          </div>
          <Button
            className="h-10 min-w-[146px] shrink-0 rounded-full px-4 text-sm"
            disabled={!canConfirm || busy}
            onClick={confirm}
          >
            {buttonLabel}
...
function PaymentOption({
  ...
  return (
    ...
    >
      ...
      <span
        className={`grid size-12 place-items-center rounded-2xl transition-colors duration-300 sm:size-14 ${selected ? "bg-primary text-primary-foreground" : "bg-primary-soft text-primary group-hover:bg-primary group-hover:text-primary-foreground"}`}
      >
