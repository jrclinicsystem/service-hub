...
function ProfessionalAgenda() {
  ...
  const addTime = async () => {
    ...
    setSavingTime(false);
    if (slotError) return toast.error(slotError.message);
      ...
      <main className="mx-auto max-w-5xl px-4 pb-16 pt-6 sm:px-8 sm:pt-10">
        ...
        <section className="mt-7 rounded-3xl border border-border bg-card p-5 shadow-soft sm:p-6">
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            {data.slots.length === 0 ? <p className="col-span-full rounded-2xl border border-dashed border-border p-5 text-center text-sm text-muted-foreground">Nenhum horário cadastrado.</p> : data.slots.map((slot: any) => <div key={slot.id} className="flex items-center justify-between rounded-2xl border border-border px-4 py-3"><div><p className="font-semibold tabular-nums">{slot.slot}</p><p className="text-[10px] text-muted-foreground">{slot.is_available ? "Disponível" : "Pausado"}</p></div><div className="flex items-center gap-2"><Switch checked={slot.is_available} onCheckedChange={async (checked) => { const { error: updateError } = await db.from("professional_time_slots").update({ is_available: checked, updated_at: new Date().toISOString() }).eq("id", slot.id); if (updateError) return toast.error(updateError.message); await refetch(); }} /><Button type="button" variant="ghost" size="icon" className="text-destructive" onClick={async () => { if (!window.confirm(`Remover o horário ${slot.slot}?`)) return; const { error: removeError } = await db.from("professional_time_slots").delete().eq("id", slot.id); if (removeError) return toast.error(removeError.message); toast.success("Horário removido."); await refetch(); }}><Trash2 className="size-4" /></Button></div></div>)}
          </div>
        </section>
...
function ProfessionalAppointmentCard({ appointment, onSaved }: any) {
  ...
  const confirmCommitment = async () => {
    ...
    setBusy(false);
    if (error) return toast.error("Não foi possível confirmar o compromisso.");
