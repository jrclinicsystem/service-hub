...
export function AdminAppointmentsWorkspace({
  ...
  const completeAttendance = async (appointment: any) => {
    ...
    toast.success("Atendimento concluído.", {
      description: "O valor agora foi contabilizado na receita.",
    });
    setSelected((current: any) =>
  ...
  const removeAppointment = async (appointment: any) => {
    if (
      !window.confirm(
        `Apagar o agendamento de ${appointment.patient_name}? Esta ação não pode ser desfeita.`,
      )
    )
      return;
...
function AdminAppointmentCard({ appointment, onOpen, onDelete, onAttended, deleting }: any) {
  ...
  return (
    <article
      className={`rounded-2xl border p-4 text-left shadow-soft transition hover:shadow-md ${cardClass}`}
    >
      ...
      {appointment.status === "atendido" ? (
        <div className="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-center text-[11px] font-semibold text-emerald-800">
...
function CreateAppointmentDialog({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!active) return;
      const firstError = [serviceResult, professionalResult, linkResult, clientResult].find(
        (result) => result.error,
      )?.error;
  ...
  const availableProfessionals = useMemo(() => {
    ...
    const allowed = new Set(
      links.filter((link) => link.service_id === serviceId).map((link) => link.professional_id),
    );
  ...
  const createAppointment = async () => {
    ...
    const validLink = links.some(
      (link) => link.service_id === serviceId && link.professional_id === professionalId,
    );
    if (!validLink) {
      toast.error("Esse profissional não atende o serviço selecionado.");
      return;
    }
    ...
    toast.success("Agendamento enviado para confirmação da profissional.", {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[92dvh] w-[calc(100%-1rem)] overflow-y-auto rounded-3xl p-5 sm:max-w-2xl sm:p-6">
        <DialogHeader>
          ...
          <DialogDescription>
            O agendamento será criado como aguardando confirmação da profissional e com pagamento
            presencial.
          </DialogDescription>
        </DialogHeader>
        <div className="mt-2 grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5 sm:col-span-2">
            ...
            <Select
              ...
              onValueChange={(value) => {
                ...
                setAppointmentValue(service ? String(Number(service.price ?? 0).toFixed(2)) : "");
              }}
          ...
          <div className="space-y-1.5 sm:col-span-2">
            <Label htmlFor="admin-appointment-value">Valor do atendimento *</Label>
            <Input
            ...
            />
            <p className="text-[11px] text-muted-foreground">
              O valor padrão do serviço é preenchido automaticamente. Altere aqui para aplicar
              qualquer desconto ou valor combinado sem mudar o catálogo.
            </p>
          </div>
          <div className="space-y-1.5 sm:col-span-2">
            ...
            <Select
              ...
              disabled={saving || !serviceId || availableProfessionals.length === 0}
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={
                    !serviceId ? "Escolha primeiro o serviço" : "Selecione o profissional"
                  }
                />
              </SelectTrigger>
              <SelectContent>
        ...
        <DialogFooter className="mt-4 gap-2 sm:gap-0">
          ...
          <Button onClick={createAppointment} disabled={saving || loadingCatalog}>
...
function SmallInfo({
  ...
  return (
    <div className="min-w-0">
      ...
      <p className={`mt-0.5 truncate text-xs font-medium ${accent ? "text-primary" : ""}`}>
        {value}
      </p>
    </div>
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92dvh] w-[calc(100%-1rem)] overflow-y-auto rounded-3xl p-5 sm:max-w-2xl sm:p-6">
        <DialogHeader>
          ...
          <DialogDescription>
            {appointment.service?.name ?? "Atendimento"} · {formatDate(appointment.scheduled_date)}{" "}
        ...
        {appointment.status === "atendido" ? (
          <div className="mt-4 rounded-xl bg-emerald-50 px-3 py-2 text-center text-xs font-semibold text-emerald-800">
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onLater()}>
      <DialogContent className="w-[calc(100%-1rem)] rounded-3xl p-5 sm:max-w-md sm:p-6">
        <DialogHeader>
          <span className="mb-2 grid size-12 place-items-center rounded-2xl bg-primary-soft text-primary">
        ...
        {hasWhatsApp ? (
          <Button
            variant="outline"
            className="mt-3 w-full rounded-xl border-emerald-600/40 text-emerald-700"
            onClick={() => openAppointmentWhatsApp(appointment, "confirmation")}
          >
            <MessageCircle className="size-4" /> Confirmar pelo WhatsApp
          </Button>
        ...
        <DialogFooter className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3 sm:space-x-0">
          <Button variant="outline" disabled={busy} onClick={onLater}>
            Depois
          </Button>
          <Button variant="destructive" disabled={busy} onClick={onCancel}>
            Cancelar
