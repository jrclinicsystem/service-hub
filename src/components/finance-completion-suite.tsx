...
async function loadLookups() {
  const results = await Promise.all([
    ...
    db.from("accounts_receivable_with_status").select("*").order("due_date", { ascending: true }),
...
export function FinanceCompletionSuite() {
  ...
  const report = useQuery({
    ...
    queryFn: () =>
      loadReport({ from, to, professional, service, method, category, center, status }),
  ...
  const mappingMap = useMemo(
    () =>
      new Map<string, string>(
        (lookups.data?.mappings ?? []).map((row: any) => [row.service_id, row.cost_center_id]),
      ),
  ...
  const cashMethodId = (lookups.data?.methods ?? []).find((item: any) => item.code === "cash")?.id ?? "";
  ...
  return (
    <section className="mx-auto max-w-[1500px] px-5 pb-12 lg:px-8">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <AlertList
          title="Contas a pagar — alertas"
          rows={lookups.data?.payables ?? []}
          kind="pay"
        />
        <AlertList
          ...
          <div className="mt-4 space-y-2">
            {centerRows.map((row: any) => (
              <div key={row.report_key} className="rounded-2xl border border-border p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Faturamento {money(row.gross_amount)} · Despesas {money(row.expense_amount)} ·
                  Comissões {money(row.commission_amount)}
                </p>
              </div>
            ))}
          </div>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {row.procedures_count} atendimento(s)
                    </p>
                  </div>
