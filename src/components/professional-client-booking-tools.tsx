...
async function loadProfessionalTools(professionalId: string) {
  const [clientsResult, linksResult] = await Promise.all([
    db
      .from("clients")
      .select("id,name,whatsapp,email,birth_date,is_active")
      .eq("is_active", true)
      .order("name"),
...
export function ProfessionalClientBookingTools({
  ...
  const filteredClients = useMemo(() => {
    ...
    const source = term
      ? clients.filter((client: any) =>
          [client.name, client.whatsapp, client.email].some((value) =>
            String(value ?? "")
              .toLowerCase()
              .includes(term),
          ),
  ...
  useEffect(() => {
    ...
    void db
      .rpc("get_professional_booking_slots", { _professional_id: professionalId, _date: date })
      .then(({ data, error }: any) => {
        if (cancelled) return;
        if (error) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`jrclinic-professional-booking-slots-${professionalId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  const createClient = async () => {
    ...
    if (!clientBirthDate || clientBirthDate > todayIso()) {
    ...
    <section className="mt-4 rounded-3xl border border-border bg-card p-4 shadow-soft sm:mt-7 sm:p-6">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Cadastre um cliente ou selecione alguém já salvo para lançar um atendimento diretamente na
          sua agenda.
        </p>
      </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-border bg-background p-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">Cadastrar cliente</h3>
          </div>
        ...
        <div className="rounded-2xl border border-border bg-background p-4">
          ...
          <div className="mt-4">
            ...
            <div className="relative mt-1.5">
              ...
              <Input
                ...
                placeholder={
                  query.isLoading ? "Carregando clientes..." : "Buscar nome, WhatsApp ou e-mail..."
                }
                className="pl-9"
              />
            </div>
            {!selectedClientId ? (
              <div className="mt-2 max-h-36 space-y-1 overflow-y-auto rounded-xl border border-border p-1.5">
                {filteredClients.length ? (
                  filteredClients.map((client: any) => (
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 rounded-lg px-2.5 py-2 text-left hover:bg-secondary/60"
                    >
              ...
              <div className="mt-2 flex items-center justify-between rounded-xl bg-primary-soft px-3 py-2">
          ...
          <div className="mt-3">
            ...
            <Select
              ...
              onValueChange={(value) => {
                setServiceId(value);
                const service = services.find((item: any) => item.id === value);
          ...
          <div className="mt-3">
            ...
            <p className="mt-1 text-[10px] text-muted-foreground">
              O preço padrão é preenchido automaticamente, mas você pode alterar livremente para
              aplicar desconto ou valor combinado.
            </p>
          </div>
            ...
            <div>
              ...
              <Select value={time} onValueChange={setTime} disabled={slotsLoading}>
                <SelectTrigger className="mt-1.5">
                  <SelectValue
                    placeholder={
                      slotsLoading ? "Carregando..." : slots.length ? "Selecione" : "Sem horário"
                    }
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          <Button
            ...
            disabled={savingAppointment || slotsLoading || !selectedClientId || !serviceId || !time}
          >
            {savingAppointment ? "Salvando..." : "Criar agendamento"}
          </Button>
        </div>
      </div>
    </section>
