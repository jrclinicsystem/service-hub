...
function printReport(entries: any[], expenses: any[], from: string, to: string) {
  ...
  const entryRows = entries
    .map(
      (row) =>
        `<tr><td>${formatDate(row.business_date)}</td><td>${row.patient_name_snapshot ?? ""}</td><td>${row.professional_name_snapshot ?? ""}</td><td>${row.service_name_snapshot ?? ""}</td><td>${money(row.charged_amount)}</td><td>${money(row.net_amount)}</td></tr>`,
    )
    .join("");
  const expenseRows = expenses
    .map(
      (row) =>
        `<tr><td>${formatDate(row.expense_date)}</td><td>${row.description ?? ""}</td><td>${money(row.amount)}</td></tr>`,
    )
    .join("");
